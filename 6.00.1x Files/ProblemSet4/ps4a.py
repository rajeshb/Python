...
def getWordScore(word, n):
    ...
    #print "Returning ", points, " points for word :", word
    return points
#
...
def isValidWord(word, hand, wordList):
    ...
    handCopy = hand.copy()
    for c in word:
        # If the letter is not part of the hand, return False
        if not (c in handCopy.keys()):
            return False
            
        # If the letter from word is used more than available in hand, 
        # return False
        if handCopy[c] > 0:
            handCopy[c] -= 1
        else:
    ...
    #If the word is part of the word list, then True. Else, False
    if word in wordList:
        return True
    return False
#
    ...
    while calculateHandlen(handCopy) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                currentWordPoints = getWordScore(inputWord,n)
                totalPoints += currentWordPoints
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                print('"%s" earned %d points. Total: %d' % (inputWord, currentWordPoints, totalPoints))
                # Update the hand 
                handCopy = updateHand(handCopy, inputWord)
